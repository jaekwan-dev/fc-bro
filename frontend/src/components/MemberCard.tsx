...
export function MemberCard({ member, onClick }: MemberCardProps) {
  ...
  const subPositions = member.subPosition
    ? Array.isArray(member.subPosition)
      ? member.subPosition
      : [member.subPosition]
    : null;
  ...
  const cardContent = (
    <Card
      className={`mb-2 hover:shadow-md transition-shadow duration-200 bg-gradient-to-r cursor-pointer ${getPositionGradient(member.mainPosition)} ${member.name === '박민수' ? 'relative overflow-hidden' : ''}`}
      style={
        member.name === '박민수'
          ? {
              background:
                'linear-gradient(120deg,rgb(244, 250, 255) 40%,rgb(51, 116, 190) 100%)',
            }
          : {}
      }
    >
          ...
          <div className="flex-1 space-y-3 relative">
            <div className="flex items-center gap-2 mb-1">
              ...
              {member.injuries &&
                ...
                member.injuries.some(injury => injury.isActive) && (
                  <Badge
                    variant="destructive"
                ...
                className={`text-xs font-bold px-2 py-0.5
                  ${
                    member.mainLevel === '프로'
                      ...
                      : member.mainLevel === '세미프로'
                        ? 'bg-purple-500 text-white'
                        : member.mainLevel === '아마추어'
  ...
  if (onClick) {
    return <div onClick={() => onClick(member)}>{cardContent}</div>;
  }
