...
export function MemberEditForm({ member, onSuccess, onCancel }: MemberEditFormProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.birthYear || formData.birthYear < 1970 || formData.birthYear > 2010) {
          ...
          <Card className="border-0 shadow-lg">
            <CardContent className="px-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    {validationErrors.birthYear && (
                      <p className="text-red-500 text-sm mt-1">{validationErrors.birthYear}</p>
                    )}
                  </div>
                  <div>
                ...
                <div>
                  <Label className="text-sm font-medium text-gray-700 mb-3 block">주발</Label>
                  <div className="grid grid-cols-3 gap-3">
                    {feet.map((foot) => (
                      <button
                        key={foot}
                        type="button"
                        onClick={() => setFormData({ ...formData, preferredFoot: foot as "오른발" | "왼발" | "양발" })}
